...
void Air_InfraredCondition_SendData(void)
{
	if(1 == gDeviceRelate.UseFlag)                      //使能状态
  {
		...
		Msg_Send((ObjectType_t)gDeviceRelate.RelateType , *(u32*)&(gDeviceRelate.ObjectID ),*(u16*)&gDeviceRelate.AssistID, \
				 gDeviceRelate.UnitNum , 0x50 , 6 , pData);
    }
}
/*****************************************************************************
函 数 名:  	Air_FJPG_SendData
功能说明:  	绑定对象为风机盘管时命令转发
参    数:  	无
返 回 值:  	无
*****************************************************************************/
void Air_FJPG_SendData(void)
{
	u8 pData[10];
	if(1 == gDeviceRelate.UseFlag)                      //使能状态
	{		
		if(gDeviceRelate.SelfAdjust == 1) //面板自反馈调节开启,温度自解析
		{
			FJPG_Data_Config();
		}
		else //面板自反馈调节关闭，直接发数据
		{
			pData[0] = gAirStatus.power; 
			pData[1] = gAirStatus.temp ;
			pData[2] = gAirStatus.mode ;
			pData[3] = gAirStatus.speed ;
			pData[4] = gAirStatus.wind ;
			Msg_Send((ObjectType_t)gDeviceRelate.RelateType , *(u32*)&(gDeviceRelate.ObjectID ),
					*(u16*)&gDeviceRelate.AssistID,gDeviceRelate.UnitNum , 0x50 , 5 , pData);
		}
	}
}
/*****************************************************************************
函 数 名:  	FJPG_Data_Config
功能说明:  	针对面板自身反馈的数据解析
参    数:  	status 空调的控制状态数据
返 回 值:  	无
*****************************************************************************/
void FJPG_Data_Config(void)
{
	u8 pData[10];
  static u8 xData[10]={0};
	u8 temp = (gAirCondition.temp >= gAirStatus.temp) ?
		(gAirCondition.temp - gAirStatus.temp):(gAirStatus.temp - gAirCondition.temp);
	Thread_Login(ONCEDELAY , 0 , 40000 , FJPG_Data_Config); //每40s判断调节一次 
	
	pData[0] = gAirStatus.power; //开关
	if(gAirCondition.temp >= gAirStatus.temp) pData[1] = 0 ; //降温表示
	else pData[1] = 1 ; //升温表示
	pData[2] = gAirStatus.mode ;
	if(gAirStatus.speed == 0x00)//自动风速
	{
		if(temp > 5)
		{
			pData[3] = 0x03 ; //大风
	...
	else if(gAirStatus.speed < 0x04)
	{
		pData[3] = gAirStatus.speed ; //固定风速
	}
	pData[4] = gAirStatus.wind ;
	if(1 == memcmp(pData , xData , 5)) //前后两次产生的调温数据不同
	{
		memcpy(xData , pData , 5);
		Msg_Send((ObjectType_t)gDeviceRelate.RelateType , *(u32*)&(gDeviceRelate.ObjectID ),
		*(u16*)&gDeviceRelate.AssistID,gDeviceRelate.UnitNum , 0x51 , 5 , pData);
	}
}
/*****************************************************************************
函 数 名:  	CheckBindAddr
