...
void BindObject_EEPROMInit(void)
{
	...
	EEPROM_Write(BINDOBJECT_ADDR, sizeof(gBindObject), (u8*)&gBindObject);		
		gDeviceRelate.SelfAdjust = 0 ;
		gDeviceRelate.RelateType  = 0 ;
    gDeviceRelate.ObjectID[0] = 0 ;
    gDeviceRelate.ObjectID[1] = 0 ;
    gDeviceRelate.ObjectID[2] = 0 ;
    gDeviceRelate.ObjectID[3] = 0 ;
    gDeviceRelate.AssistID[0] = 0 ;
    gDeviceRelate.AssistID[1] = 0 ;
    gDeviceRelate.UnitNum     = 0 ;
...
void BindObject_Init(void)
{
	...
	if(gBindObject.ObjectType > 0x03  //最多到0x03，风机盘管 yanhuan adding 2015/11/18
		|| gBindObject.Addr > 64 
		|| 0 == gBindObject.Addr)
	{
...
时	  间：  2015/11/18
*******************************************************************************/
MsgResult_t Set_SelfAdjust_Attr(UnitPara_t *pdata)
{
	if(gBindObject.ObjectType != 0x03) //只对风机操作有效
	{
		return PARA_MEANING_ERR ;
	}
	if((gDeviceRelate.SelfAdjust = pdata->data[0]) == 0) //面板自反馈关闭
	{
		Thread_Logout(FJPG_Data_Config);
	}
	else //开启
	{
		Air_CmdDelaySend(1, 100);
	}		
	EEPROM_Write(DEVICE_RELATE_ADDR, sizeof(gDeviceRelate), (u8*)&gDeviceRelate);//保存状态
	return COMPLETE;
}
/*******************************************************************************
函 数 名：  Get_SelfAdjust_Attr
功能说明：  获取面板自反馈调节属性
参    数：  *pdata     
            *rlen       -- 返回长度
            *rpara      -- 返回数据
...
时	  间：  2015/11/18
*******************************************************************************/
MsgResult_t Get_SelfAdjust_Attr(UnitPara_t *pdata, u8 *rlen, u8 *rpara)
{
	if(gBindObject.ObjectType != 0x03) //只对风机操作有效
	{
		return PARA_MEANING_ERR ;
	...
	*rlen = 1 ;
	*rpara = gDeviceRelate.SelfAdjust ;
	return COMPLETE;
}
/*******************************************************************************
...
{
    if((gDeviceRelate.UseFlag = pdata->data[0]) == 0) //设备关联关闭
		{
			Thread_Logout(FJPG_Data_Config);
		}	  
	EEPROM_Write(DEVICE_RELATE_ADDR, sizeof(gDeviceRelate), (u8*)&gDeviceRelate);//保存状态
